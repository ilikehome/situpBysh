    ...
    if len(people) <= 5:
        ...
        for i, sub_rect in enumerate(sub_rectangles):
            sub_people = [person for person in people if
                          sub_rect[0] <= person_center[0] < sub_rect[2] and sub_rect[1] <= person_center[1] <
                          sub_rect[3]]
            if len(sub_people) == 1:
                sub_results[i] = sub_people[0]
            elif len(sub_people) > 1:
                if i in [1, 3, 5]:
                    sub_results[i] = min(sub_people, key=lambda x: person_center[1])
                elif i in [2, 4]:
                    sub_results[i] = max(sub_people, key=lambda x: person_center[1])
